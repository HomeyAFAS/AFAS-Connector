...
module.exports = {
	createAutocompleteActions: function () {
		Homey.manager('flow').on('action.get_connector_better_logic.arg_name.get_connector', function( callback, args ){
		...
		Homey.manager('flow').on('action.get_connector_speech.arg_name.get_connector', function( callback, args ){
			connector.getConnector( "metainfo", "", function( err, response ){
				if (err) Homey.log(err); return callback( 'No connectors found', null );
